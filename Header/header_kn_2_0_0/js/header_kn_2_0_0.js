const eventPC = (data) => {
    const header_kn_2_0_0_elmHover = document.querySelectorAll('.header_kn_2_0_0__navItem a');
    const header_kn_2_0_0_elmBg = document.querySelector('.header_kn_2_0_0__bg');
    for(let i = 0; i < header_kn_2_0_0_elmHover.length; i++){
        header_kn_2_0_0_elmHover[i].addEventListener('mouseover', () => {
            for(let i = 0; i < header_kn_2_0_0_elmHover.length; i++){
                const header_kn_2_0_0_elmId = header_kn_2_0_0_elmHover[i].getAttribute('data-id');
                if(header_kn_2_0_0_elmId !== null){
                    document.getElementById(header_kn_2_0_0_elmId).innerHTML = '';
                }
            }
            const header_kn_2_0_0_elmId = header_kn_2_0_0_elmHover[i].getAttribute('data-id');
            const innerData = data.filter(item => item.label === header_kn_2_0_0_elmId);
            if(innerData.length > 0){
                document.getElementById(header_kn_2_0_0_elmId).innerHTML = innerData[0].layout;
                header_kn_2_0_0_elmBg.style.display = 'block';
            }
        })
    }
    header_kn_2_0_0_elmBg.addEventListener('mouseover', () => {
        for(let i = 0; i < header_kn_2_0_0_elmHover.length; i++){
            const header_kn_2_0_0_elmId = header_kn_2_0_0_elmHover[i].getAttribute('data-id');
            if(header_kn_2_0_0_elmId !== null){
                document.getElementById(header_kn_2_0_0_elmId).innerHTML = '';
...
resizeHeader(header_kn_2_0_0_data);
